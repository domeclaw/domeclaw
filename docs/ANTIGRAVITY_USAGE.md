# Using Antigravity Provider in DomeClaw

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in DomeClaw.
...
```bash
domeclaw auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where DomeClaw is waiting.

DomeClaw will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.domeclaw/config.json` or override it via the CLI:

```bash
# Override for a single command
domeclaw agent -m "Hello" --model claude-opus-4-6-thinking
```
...
2.  **Authentication persistence**: 
    ...
    ```bash
    scp ~/.domeclaw/auth.json user@your-server:~/.domeclaw/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. DomeClaw will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `domeclaw auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
