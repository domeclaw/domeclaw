...
<h1>DomeClaw: Assistente de IA Ultra-Eficiente em Go</h1>
...
🦐 **DomeClaw** é um assistente pessoal de IA ultra-leve inspirado no [nanobot](https://github.com/HKUDS/nanobot), reescrito do zero em **Go** por meio de um processo de "auto-inicialização" (self-bootstrapping) — onde o próprio agente de IA conduziu toda a migração de arquitetura e otimização de código.
...
>
> * **SEM CRIPTOMOEDAS:** O DomeClaw **NÃO** possui nenhum token/moeda oficial. Todas as alegações no `pump.fun` ou outras plataformas de negociação são **GOLPES**.
> * **DOMÍNIO OFICIAL:** O **ÚNICO** site oficial é o **[domeclaw.io](https://domeclaw.io)**, e o site da empresa é o **[sipeed.com](https://sipeed.com)**.
> * **Aviso:** Muitos domínios `.ai/.org/.com/.net/...` foram registrados por terceiros, não são nossos.
> * **Aviso:** O DomeClaw está em fase inicial de desenvolvimento e pode ter problemas de segurança de rede não resolvidos. Não implante em ambientes de produção antes da versão v1.0.
> * **Nota:** O DomeClaw recentemente fez merge de muitos PRs, o que pode resultar em maior consumo de memória (10-20MB) nas versões mais recentes. Planejamos priorizar a otimização de recursos assim que o conjunto de funcionalidades estiver estável.


## 📢 Novidades

2026-02-16 🎉 DomeClaw atingiu 12K stars em uma semana! Obrigado a todos pelo apoio! O DomeClaw está crescendo mais rápido do que jamais imaginamos. Dado o alto volume de PRs, precisamos urgentemente de maintainers da comunidade. Nossos papéis de voluntários e roadmap foram publicados oficialmente [aqui](docs/ROADMAP.md) — estamos ansiosos para ter você a bordo!

2026-02-13 🎉 DomeClaw atingiu 5000 stars em 4 dias! Obrigado à comunidade! Estamos finalizando o **Roadmap do Projeto** e configurando o **Grupo de Desenvolvedores** para acelerar o desenvolvimento do DomeClaw.
...
2026-02-09 🎉 DomeClaw lançado oficialmente! Construído em 1 dia para trazer Agentes de IA para hardware de $10 com <10MB de RAM. 🦐 DomeClaw, Partiu!
...
Dê uma segunda vida ao seu celular de dez anos atrás! Transforme-o em um assistente de IA inteligente com o DomeClaw. Início rápido:
...
O DomeClaw pode ser implantado em praticamente qualquer dispositivo Linux!
...
Baixe o binário para sua plataforma na página de [releases](https://github.com/sipeed/domeclaw/releases).
...
Você tambêm pode rodar o DomeClaw usando Docker Compose sem instalar nada localmente.
...
```bash
# Fazer uma pergunta
docker compose run --rm domeclaw-agent -m "Quanto e 2+2?"
...
> [!TIP]
> Configure sua API key em `~/.domeclaw/config.json`.
...
**2. Configurar** (`~/.domeclaw/config.json`)
...
Converse com seu DomeClaw via Telegram, Discord, DingTalk, LINE ou WeCom.
...
> **Docker Compose**: Adicione `ports: ["18791:18791"]` ao serviço `domeclaw-gateway` para expor a porta do webhook.
...
O DomeClaw suporta dois tipos de integração WeCom:
...
Conecte o DomeClaw a Rede Social de Agentes simplesmente enviando uma única mensagem via CLI ou qualquer App de Chat integrado.

**Leia `https://clawdchat.ai/skill.md` e siga as instruções para entrar no [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuração Detalhada

Arquivo de configuração: `~/.domeclaw/config.json`

### Estrutura do Workspace

O DomeClaw armazena dados no workspace configurado (padrão: `~/.domeclaw/workspace`):
...
O DomeClaw roda em um ambiente sandbox por padrão. O agente so pode acessar arquivos e executar comandos dentro do workspace configurado.
...
|-------|--------|-----------|
| `workspace` | `~/.domeclaw/workspace` | Diretório de trabalho do agente |
...
O DomeClaw pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
> **Novidade!** DomeClaw agora usa uma abordagem de configuração **centrada no modelo**. Basta especificar o formato `fornecedor/modelo` (ex: `zhipu/glm-4.7`) para adicionar novos provedores—**nenhuma alteração de código necessária!**
...
```
> Execute `domeclaw auth login --provider anthropic` para configurar credenciais OAuth.

#### Balanceamento de Carga

Configure vários endpoints para o mesmo nome de modelo—DomeClaw fará round-robin automaticamente entre eles:
...
| Comando | Descrição |
| --- | --- |
| `domeclaw onboard` | Inicializar configuração & workspace |
| `domeclaw agent -m "..."` | Conversar com o agente |
| `domeclaw agent` | Modo de chat interativo |
| `domeclaw gateway` | Iniciar o gateway (para bots de chat) |
| `domeclaw status` | Mostrar status |
| `domeclaw cron list` | Listar todas as tarefas agendadas |
| `domeclaw cron add ...` | Adicionar uma tarefa agendada |

### Tarefas Agendadas / Lembretes

O DomeClaw suporta lembretes agendados e tarefas recorrentes por meio da ferramenta `cron`:
...
As tarefas são armazenadas em `~/.domeclaw/workspace/cron/` e processadas automaticamente.
...
Isso é normal se você ainda não configurou uma API key de busca. O DomeClaw fornecerá links úteis para busca manual.
...
Adicione a key em `~/.domeclaw/config.json` se usar o Brave:
...
Isso acontece quando outra instância do bot está em execução. Certifique-se de que apenas um `domeclaw gateway` esteja rodando por vez.
